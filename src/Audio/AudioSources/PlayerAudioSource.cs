...
namespace RPVoiceChat.Audio.AudioSources
{
    public class PlayerAudioSource : BaseAudioSource
    {
        ...
        public PlayerAudioSource(ICoreClientAPI capi, ClientSettingsRepository clientSettingsRepo, IPlayer player) : base(capi, clientSettingsRepo, player.Entity?.SidedPos?.XYZFloat)
        {
            this.player = player;

            OnSourceStartPlaying += OnPlayerSpeaking;
        ...
        private void OnPlayerSpeaking()
        {
            PlayerNameTagRenderer.UpdatePlayerNameTag(player, true);
        }

        private void OnPlayerStoppedSpeaking()
        {
